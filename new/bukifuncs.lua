...
function BukiFuncs:JumpPower(Value)
    Player.Character.Humanoid.JumpPower = Value
end

function BukiFuncs:hash(string)
    ...
    local function preproc(string, len)
            local extra = 64 - ((len + 1 + 8) % 64)
            len = num2s(8 * len, 8)
            string = string .. "\128" .. string.rep("\0", extra) .. len
            return string
    end
    ...
    local function digestblock(string, i, H)
            local w = {}
            for j = 1, 16 do w[j] = s232num(string, i + (j - 1) * 4) end
            for j = 17, 64 do
                    local v = w[j - 15]
    ...
    string = preproc(string, #string)
    local H = initH256({})
    for i = 1, #string, 64 do digestblock(string, i, H) end
...
function BukiFuncs:Msgbox(string, mod)
    local Lowered = string.lower(mod)
    if Lowered == "info" then
        rconsoleinfo(string);
    elseif Lowered == "warn" then
        rconsolewarn(string);
